...
Postman показва мрежова информация, когато вашият API върне отговор. Задръжте курсора на мишката върху иконата на мрежата ![](</courses/assets/image%20(37).png>), за да получите локалния и отдалечения IP адрес за заявката, която сте изпратили.
...
Този тест проверява дали сървърът връща HTTP статус код 200, което означава, че заявката е изпълнена успешно.
pm.test() създава тест в Postman и приема име на теста и функция, която съдържа проверките.
pm.response представлява отговора от сървъра, а to.have.status(200) проверява дали статус кодът на отговора е точно 200.

 - Тест за поле в JSON отговор
...
```
Този тест проверява дали JSON отговорът съдържа поле success със стойност true.
pm.response.json() преобразува отговора от сървъра в JavaScript обект, за да може да се достъпват неговите полета.
pm.expect() се използва за проверка на стойности, а to.be.true валидира, че стойността на success е true.

- Тест за стойност в XML отговор

```JavaScript
pm.test("Отговорът трябва да съдържа person с age = 26", function () {
    var jsonData = xml2Json(pm.response.text());
    pm.expect(jsonData.person.age).to.eq("26");
});
```
Този тест проверява дали XML отговорът съдържа елемент person с поле age, чиято стойност е 26.
pm.response.text() връща отговора като текст, защото XML не може директно да се обработва като JSON.
xml2Json() преобразува XML структурата в JSON обект, за да може да се достъпват елементите чрез свойства.
pm.expect(...).to.eq() сравнява стойността на age с очакваната стойност "26".
