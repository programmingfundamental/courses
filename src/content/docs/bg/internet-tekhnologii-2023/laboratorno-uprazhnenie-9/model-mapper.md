...
<figure><img src="/courses/assets/image%20(137).png" alt=""><figcaption></figcaption></figure>
...
ModelMapper поддържа преобразуване на колекции от обекти (напр. List, Set). Поради механизма type erasure в Java, при работа с генерични типове е необходимо изрично да се укаже типът на целевата колекция чрез TypeToken.

### Type erasure (изтриване на типове)

Type erasure е механизъм в Java, при който информацията за генеричните типове (например List<String>, List<Task>) се премахва по време на компилация. В резултат на това по време на изпълнение JVM не разпознава конкретния тип на елементите в колекцията.

ModelMapper не може коректно да мапне колекция към типизиран списък само с List.class, защото Java изтрива generic типовете по време на изпълнение.

Затова при мапване на колекции трябва да се използва TypeToken, за да се запази информацията за типа на елементите:
