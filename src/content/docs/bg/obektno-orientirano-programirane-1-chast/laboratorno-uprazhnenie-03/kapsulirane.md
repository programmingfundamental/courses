...
Това понятие се различава от ключовата дума **`interface`**, с която Java декларира отделен тип и договор за поведение. Един клас има публичен API дори когато не имплементира Java интерфейс. Конструкцията `interface` се разглежда в [упражнение 4](/courses/bg/obektno-orientirano-programirane-1-chast/laboratorno-uprazhnenie-04/abstrakciya-abstraktni-klasove-i-interfeisi/).
