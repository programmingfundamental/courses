...
И при двата подхода `final` референция не прави сочения обект неизменяем. За масиви се използват защитни копия при приемане и връщане. Пример е даден в [Видове класове](../laboratorno-uprazhnenie-02/vidove-klasove.md#повърхностна-неизменяемост-и-защитно-копиране).
...
Тук **интерфейс** означава начинът за използване на класа — неговият публичен API. Например клиентът на банкова сметка извиква `deposit(...)` и `getBalance()`, без да знае как се съхранява балансът. В общия случай публичният API може да включва и публични полета и вложени типове; при капсулираните примери полетата са частни.

Това понятие се различава от ключовата дума **`interface`**, с която Java декларира отделен тип и договор за поведение. Един клас има публичен API дори когато не имплементира Java интерфейс. Конструкцията `interface` се разглежда в [упражнение 4](../laboratorno-uprazhnenie-04/abstrakciya-abstraktni-klasove-i-interfeisi.md).
