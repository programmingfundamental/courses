...
```java
Student first = new Student("Ivan", 12345);
Student second = first;
second.name = "Maria";
...
Присвояването `second = first` копира референцията. Двете променливи сочат към един и същ обект, затова промяната през едната се вижда и през другата. `new Student(...)` създава отделен обект. При референции `==` проверява дали се сочи към същия обект; `equals()` може да сравнява съдържание според реализацията на класа.

Стойността `null` означава, че референцията не сочи към обект. Преди достъп до поле или извикване на нестатичен метод трябва да има действителен обект.
