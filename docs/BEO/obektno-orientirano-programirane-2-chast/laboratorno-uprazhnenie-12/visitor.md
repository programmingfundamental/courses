...
Ползата от този модел е, че ако логиката на операцията се промени, тогава трябва да направим промяна само в имплементацията на посетителя, вместо да го правим във всички класове елементи. Друго предимство е, че добавянето на ново поведение към елементите е лесно и предполага само нова конкретна имплементация на посетителя.

#### Ограничения на модела на посетителя

Недостатъкът на модела на посетителите е, че трябва да знаем типа на връщане на методите за посещение по време на проектирането, в противен случай ще трябва да променим интерфейса и всички негови реализации. Друг недостатък е трудното добавяне на нови елементи - ако в примера по-горе добавим нова конкретна имплементация на ItemElement ще трябва да добавим метод в интерфейса, което ще доведе и до модификация на съществуващите конкретни посетители.
