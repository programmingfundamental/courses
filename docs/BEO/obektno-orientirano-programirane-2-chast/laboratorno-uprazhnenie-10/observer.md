...
Да се разработи система за публикуване на съобщения.

При публикуване на ново съобщение всички абонирани потребители трябва да бъдат уведомени за него.


### Решение без използване на шаблона

Един възможен подход е класът Topic да създава и уведомява конкретните потребители директно:

```java
// class TopicSubscriber defined in advance

public class Topic {

    private TopicSubscriber firstSubscriber =
            new TopicSubscriber("First subscriber");

    private TopicSubscriber secondSubscriber =
            new TopicSubscriber("Second subscriber");

    public String postMessage(String message) {

        StringBuilder result = new StringBuilder();

        result.append(firstSubscriber.update(message))
              .append("\n");

        result.append(secondSubscriber.update(message));

        return result.toString();
    }
}
```
Използването изглежда така:
```java
...
При този подход класът Topic зависи пряко от конкретните класове на потребителите, които трябва да бъдат уведомени.

При добавяне на нов абониран потребител е необходимо класът Topic да бъде модифициран. Аналогично, ако някой потребител трябва да спре да получава известия, отново се налага промяна в неговата реализация.

Освен това броят на получателите е фиксиран и не може да се променя динамично по време на изпълнение на програмата.

Следователно е необходимо решение, което позволява наблюдаваният обект да уведомява всички заинтересовани обекти, без да зависи от техните конкретни реализации.
