...
Метода `showAndWait` връща като резултат информация кой бутон от `Alert` прозореца е бил натиснат. Резултата е от тип `Optional<ButtonType>`
Optional<T> е клас в Java (пакет java.util), който се използва за представяне на стойност, която може да съществува или да липсва.

Optional<T> е контейнер, който:

съдържа стойност (T) → ако има
или е празен → ако няма стойност
